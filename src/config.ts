export const gameWidth = 900
export const gameHeight = 600

export const arenaRadius = (gameHeight - 50) / 2

export const poundRadius = 200

export const backgroundColor = 0x8d8682

export const circleColors = [0x2b59c3, 0xd36582]
export const circleLightColors = [0x9eb5e9, 0xe1a2b3]

export const playerSpeed = 4
export const enemySpeed = 0.75

export const jumpDuration = 0.7

export const jumpCooldownTime = 0.95

export const knockBackDuration = 0.5

export const circleRadius = 40

export const scoreTextStyle = {
  align: 'center',
  dropShadowAngle: 0,
  dropShadowBlur: 7,
  dropShadowDistance: 0,
  fill: ['white'],
  fontSize: 90,
  wordWrapWidth: 340,
  fontFamily: 'kashima',
  wordWrap: true
}

export const randomArenaPosition = () =>
  Math.random() * arenaRadius - arenaRadius / 2
